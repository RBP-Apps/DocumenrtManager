...
export default function AddDocument() {
  ...
  const fetchMasterData = async () => {
    try {
      ...
      setcompanies(Array.from(new Set(companyList))); // Remove duplicates
  ...
  const firstDocumentType = multipleFiles[0]?.type;
  const firstDocumentCategory = multipleFiles[0]?.documentType;
  const firstDocumentName = multipleFiles[0]?.name;
  ...
  const addFileRow = () => {
    setMultipleFiles([
      ...multipleFiles,
      {
        ...
        documentType: firstDocumentCategory || "Personal",
        file: null,
        entityName: "",
        needsRenewal: false,
        renewalDate: "",
            ...
            <CardContent className="space-y-6 p-4 md:p-6 bg-gray-50 ">
              {multipleFiles.map((fileItem, index) => (
                ...
                >
                  ...
                  {/* Show 3-column grid ONLY for the FIRST document (index === 0) */}
                  {index === 0 && (
                  ...
                  {/* Renewal section - shown for ALL documents */}
                  ...
                  <div className="border-t pt-3 mt-3 ">
                    ...
                    {/* Add Another Document Button - only on last document */}
                    {index === multipleFiles.length - 1 && (
                      <Button
                        type="button"
                        variant="outline"
                        onClick={addFileRow}
